    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for (int i = 0; i < userCount; i++) {
            ...
            // compute mutual followee count between 'user' and users[i] by checking
            // how many users in the network both of them follow
            int mutualCount = 0;
            for (int j = 0; j < userCount; j++) {
                String possible = users[j].getName();
                if (user.follows(possible) && users[i].follows(possible)) {
                    mutualCount++;
                }
            }
    ...
    private int followeeCount(String name) {
        int count = 0;
        for (int i = 0; i < userCount; i++) {
            // check if 'name' appears in users[i]'s followee list
